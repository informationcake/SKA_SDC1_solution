...
def do_primarybeam_correction(pbname, imagename):
    ...
    # remove 3rd and 4th axis to prevent errors with Cutout2D, aplpy, montage...
...
def convolve_regrid(imagename, ref_imagename, make_beam_plot=True):
    ...
    # fix freq info that montage overwrites. leaving this until make_cube function.
    #hdu1400 = fits.open(imagename[:-5]+'_convolved_regrid.fits', mode='update')
    #hdu1400[0].header.set('CRPIX3', 1) # Need ref pix=1
    #hdu1400[0].header.set('CRVAL3', CRVAL3) # ch0 freq
...
def make_image_cube(imagename560, imagename1400, outfilename):
    print( 'Fixing freq info in header...')
...
if __name__ == '__main__':
    ...
    #do_sourcefinding('1400mhz8hours_PBCOR_trainarea_convolved_regrid.fits')
